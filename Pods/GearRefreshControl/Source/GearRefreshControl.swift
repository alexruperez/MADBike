...
open class GearRefreshControl: UIRefreshControl {
   ...
   */
    @objc dynamic open var gearTintColor: UIColor? {
  ...
  open func scrollViewDidScroll(_ scrollView: UIScrollView) {
    ...
    [self.refreshContainerView, self.overlayView].forEach { $0.frame = refreshBounds }
...
private extension GearRefreshControl {
  func setupRefreshControl() {
    ...
    [topGear, rightGear, bottomGear, leftGear, centerGear, shadowView, overlayView].forEach { self.refreshContainerView.addSubview($0) }
