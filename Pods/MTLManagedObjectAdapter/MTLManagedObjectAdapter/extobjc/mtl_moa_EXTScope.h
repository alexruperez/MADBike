...
#define onExit \
    try {} @finally {} \
    __strong mtl_moa_cleanupBlock_t metamacro_concat(mtl_moa_exitBlock_, __LINE__) __attribute__((cleanup(mtl_moa_executeCleanupBlock), unused)) = ^
...
#define weakify(...) \
    try {} @finally {} \
    metamacro_foreach_cxt(mtl_moa_weakify_,, __weak, __VA_ARGS__)
...
#define unsafeify(...) \
    try {} @finally {} \
    metamacro_foreach_cxt(mtl_moa_weakify_,, __unsafe_unretained, __VA_ARGS__)
...
#define strongify(...) \
    ...
    metamacro_foreach(mtl_moa_strongify_,, __VA_ARGS__) \
    _Pragma("clang diagnostic pop")

/*** implementation details follow ***/
typedef void (^mtl_moa_cleanupBlock_t)();

void mtl_moa_executeCleanupBlock (__strong mtl_moa_cleanupBlock_t *block);

#define mtl_moa_weakify_(INDEX, CONTEXT, VAR) \
    CONTEXT __typeof__(VAR) metamacro_concat(VAR, _weak_) = (VAR);

#define mtl_moa_strongify_(INDEX, VAR) \
