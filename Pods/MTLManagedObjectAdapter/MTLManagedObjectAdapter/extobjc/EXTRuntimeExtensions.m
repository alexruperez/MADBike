...
mtl_moa_propertyAttributes *mtl_moa_copyPropertyAttributes (objc_property_t property) {
    ...
    mtl_moa_propertyAttributes *attributes = calloc(1, sizeof(mtl_moa_propertyAttributes) + typeLength + 1);
    if (!attributes) {
        fprintf(stderr, "ERROR: Could not allocate mtl_moa_propertyAttributes structure for attribute string \"%s\" for property %s\n", attrString, property_getName(property));
        return NULL;
    }
    ...
    while (next && *next == ',') {
        ...
        case 'C':
            attributes->memoryManagementPolicy = mtl_moa_propertyMemoryManagementPolicyCopy;
            break;

        case '&':
            attributes->memoryManagementPolicy = mtl_moa_propertyMemoryManagementPolicyRetain;
            break;
